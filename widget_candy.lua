...
V._CheckProps = function ( Widget)
	...
	if V.Themes [Props.theme] == nil then print("!!! WIDGET ERROR: _CheckProps(): Unknown theme '"..Props.theme.."'."); return end
...
-- PUBLIC: LOAD A THEME
----------------------------------------------------------------
V.LoadTheme = function ( themeFile, folderPath)
	local i, Theme
	...
	V.Themes[themeFile] = Theme
	V.Themes[themeFile].color = {255,255,255,255}

	V.print("--> Widgets.LoadTheme(): Loaded theme '"..themeFile.."'.")
end
...
V.NewButton = function (Props)
	...
	function Grp:_update()
		...
		-- TOGGLE BUTTON AND "ON"?
		if Props.toggleButton == true and Props.toggleState == true then self:_drawPressed() end
...
V.NewDragButton = function (Props)
	...
	function Grp:_setValue(event)
		...
		Props.value  = self.val + (V.Round( (self.sy - event.ly) / 10 )*step)
...
V.NewSwitch = function (Props)
	...
	function Grp:_setPos( event )
		...
		local x        = event.lx - halfSize; if x < 0 then x = 0 elseif x > size then x = size end
